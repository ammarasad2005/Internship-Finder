...
export class WorkerLifecycle {
   ...
   */
  static async runSession(sessionId: string, profileId: string, customClient?: any) {
    let heartbeatInterval: NodeJS.Timeout | null = null;
    
    // Create or use the service role client
    const supabase = customClient || createClient<Database>(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.SUPABASE_SERVICE_ROLE_KEY!
    );
    
    const costTracker = new CostTracker(sessionId, profileId, supabase);

    try {
      // 1. Mark as running
      await SessionService.updateSessionStatus(sessionId, 'running', supabase);
      await SessionService.publishEvent(sessionId, 'worker_assigned', 'Background worker picked up the session.', null, supabase);
      ...
      heartbeatInterval = setInterval(async () => {
        ...
        // await SessionService.publishEvent(sessionId, 'heartbeat', 'Worker is healthy and processing.', null, supabase);
      }, 5000);
      ...
      const executor = new ResearchExecutor(sessionId, supabase);
      ...
      await SessionService.publishEvent(sessionId, 'matching_started', 'Evaluating discovered internships against your profile...', null, supabase);
      ...
      await SessionService.publishEvent(sessionId, 'matching_completed', `AI Matching finalized. Saved ${metrics.resultsFound} potential matches.`, null, supabase);
      await SessionService.updateSessionStatus(sessionId, 'completed', supabase);

    } catch (error: any) {
      console.error(error);
      await SessionService.publishEvent(sessionId, 'fatal_error', `Worker encountered a fatal error: ${error.message}`, null, supabase);
      await SessionService.updateSessionStatus(sessionId, 'failed', supabase);
    } finally {
